...
void calculateStatistics(Node* node, int* totalStudents, int* totalAttendance) {
    if (node != NULL) {
        (*totalStudents)++;
        (*totalAttendance) += node->days_attended;
        calculateStatistics(node->left, totalStudents, totalAttendance);
        calculateStatistics(node->right, totalStudents, totalAttendance);
    }
}
...
void displayStatistics(Node* root) {
    ...
    calculateStatistics(root, &totalStudents, &totalAttendance);

    float averageAttendance = totalStudents > 0 ? (float)totalAttendance / totalStudents : 0.0;
